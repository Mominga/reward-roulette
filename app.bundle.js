...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const rulesTable = $("rulesTable"),diceBox=$("diceBox"),rollBtn=$("rollBtn"),resetBtn=$("resetBtn"),
        invList=$("invList"),resultBox=$("result"),animText=$("animText"),flash=$("flash"),flashTxt=$("flashTxt");
  ...
  const renderInv=()=>{const a=load(),m={};a.forEach(n=>m[n]=(m[n]||0)+1);invList.innerHTML=a.length?Object.entries(m).map(([n,c])=>`<div class="card">${n}${c>1?`<span class="badge">×${c}</span>`:''}</div>`).join(''):'<div class="small">まだ報酬はありません。</div>'};
  renderInv();resetBtn.onclick=()=>{localStorage.removeItem('inv');renderInv();};
  /* Dice helpers */
  const createDie=()=>{const d=document.createElement('div');d.className='die rolling';['front','back','right','left','top','bottom'].forEach(c=>{const f=document.createElement('div');f.className='face '+c;d.appendChild(f);});return d;};
  const setFace=(c,v)=>{c.className='die show-'+v;c.querySelectorAll('.face').forEach(e=>e.textContent=v);};
  ...
  /* ========= 評価 関数（順序考慮版） ========= */
  function evaluate(d){
    const sum=d.reduce((a,b)=>a+b,0),freq={};d.forEach(v=>freq[v]=(freq[v]||0)+1);
    const pairOnly=Object.values(freq).some(v=>v==2)&&Object.values(freq).every(v=>v<=2);
    const key=d.join('');
    let r=[];
    if(sum>=10)r.push('夕食を50%追加','翌朝に鼻うがい');
    if(d.some(v=>v>=5))r.push('お風呂に入る (有効48h)','顔を洗う','食器を洗う','洗濯機を回す');
    if(pairOnly)r.push('アイマスクを使う','耳栓を使う','電動自転車を利用','乾燥機能を使う');
    if(sum>=11&&sum<=14)r.push('お菓子を食べる','ギュ (ハグ) をする');
    if(sum>=4&&sum<=6)r.push('散歩に行く');if(sum==7)r.push('メルカリで不用品を1つ出品');
    if(freq[6]==3)r.push('温泉に行く');if(freq[1]==3)r.push('国内旅行 1泊2日');
    if(freq[2]==4||freq[3]==4)r.push('外食に行く');
    if(freq[4]==4)r.push('映画を鑑賞する');if(freq[5]==4)r.push('漫画喫茶に行く');if(freq[6]==4)r.push('サイクリングに行く');if(freq[1]==4)r.push('ゲーム1DAY');
    /* ── 順序固定ストレート ── */
    if(key==='1234')r.push('ルーレット旅に行く');
    if(key==='2345')r.push('○○教室ワークショップに参加');
    if(key==='3456')r.push('海外旅行に行く');
    /* ── その他固定パターン ── */
    if(['1212','2121','1221','2112','2211'].includes(key))r.push('掃除1DAY');
    if(['2341','2413','3142','3412','4123','4312'].includes(key))r.push('読書1DAY');
    if(key==='2224')r.push('●●系カフェに行く');if(key==='2255')r.push('追加で5回抽選する');if(key==='3334')r.push('登山/トレッキングに行く');if(key==='3335')r.push('泳ぎに行く');
    return r;
  }
  ...
  rollBtn.onclick=()=>{
    ...
    setTimeout(()=>{
      ...
      try{
        ...
        let html='<h2>抽選結果</h2>';
        got.forEach(n=>{
          if((map[n]||0)>=MAX_HOLD){html+=`<div class="card strike">${n}<div class="small">所持上限で獲得無し</div></div>`;}
          else{inv.push(n);map[n]=(map[n]||0)+1;html+=`<div class="card">${n}</div>`;
            if(rareList.includes(n)&&map[n]===1){flashTxt.textContent='New!! '+n+' を獲得しました！';flash.classList.add('show');rareFan();}}
        });
        resultBox.innerHTML=html;save(inv);renderInv();
      }catch(e){console.error(e);alert('エラーが発生しました');}
      finally{animText.textContent='';flash.classList.remove('show');rollBtn.disabled=false;}
    },1100);
  };
});
